...
interface DiaryApi {
    ...
    fun save(@Body diaryRequest: DiaryRequest,
             @Header("Authorization") token:String): Call<ApiResponse<DiaryEntity>>

    @GET("api/diary/getByDate")
    fun getByDate(@Header("Authorization") token:String,
                  @Query("createdAt") createdAt:String): Call<ApiResponse<DiaryEntity?>>
